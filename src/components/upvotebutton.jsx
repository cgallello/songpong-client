...
function UpvoteButton({ playlistId, upvotes, upvoted }) {
    ...
    async function upvotePlaylist() {
		if(!upvoted){
			...
			const internalResponse = await internalAxios.post('http://localhost:8000/api/playlists/' + playlistId + '/upvote', {
	...
	return (
        <button onClick={upvotePlaylist} className={localUpvoted ? "upvoteButton upvoted" : "upvoteButton"}>
            ...
            <path d="M2.2 9.4L7.7 2.06667C8.1 1.53333 8.9 1.53333 9.3 2.06667L14.8 9.4C15.2944 10.0592 14.824 11 14 11H12C11.4477 11 11 11.4477 11 12V20C11 20.5523 10.5523 21 10 21H7C6.44772 21 6 20.5523 6 20V12C6 11.4477 5.55228 11 5 11H3C2.17595 11 1.70557 10.0592 2.2 9.4Z" stroke="white" stroke-width="1.5" fill={localUpvoted ? "white" : "transparent"} />
            </svg>
            {localUpvotes}
        </button>
	);
